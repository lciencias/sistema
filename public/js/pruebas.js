...
$(document).ready(function() {	
	...
	var formPrueba = $('validateFormPrueba');
	formPrueba.on('submit', function(e){
	    e.preventDefault();
	});

	$('#validateFormPrueba').bootstrapValidator({
		...
		nombrePrueba: {
        		...
        		stringLength: {
        		    min: 3,
        		    max: 50,
        		    message: I18n._("minimoPruebaCaracteres")
        		}, 
        	    }
        	},
        	descripcionPrueba:{
...
//	$("#guardaPrueba").on("click", function (evt) {
//	    evt.preventDefault();
//	    $('#validateFormPrueba').bootstrapValidator('validate');
...
//		 form_data.append('idprueba', $("#idprueba").val());
//		 form_data.append('idmodulo', $("#idmodulo").val());
//		 form_data.append('nombre', $("#nombrePrueba").val());
//			    
//		 $.ajax({
//			type: 'POST',
//			url : url,
...
//	});
	  ...
	  $(".eliminarPrueba").on('click',function(){
		  var buffer = 0;
		  if(parseInt($("#idRegistro").val()) > 0 && parseInt($("#idModulo").val()) > 0){
			  url   = baseUrl+'catalogos/desactivaPrueba';
			  dataString = 'idModulo='+$("#idModulo").val()+'&idPrueba='+$("#idRegistro").val();
				$.ajax({
					type: 'POST',
					url : url,		
					headers: {
					...
					success: function(info){
						...
						if(parseInt(info.exito) === 1){
							...
							exito(info.msg);
                        setTimeout(function(){location.href = baseUrl+'catalogos/prueba'},tiempo);							
						}else{
							error(info.msg);
						}						
					},
	...
	//evento para restablecer a la prueba
	  $(".restablecerPrueba").on('click',function(){
		  if(parseInt($("#idRegistro").val()) > 0 && parseInt($("#idModulo").val()) > 0){
			  url   = baseUrl+'catalogos/activaPrueba';
			  dataString = 'idModulo='+$("#idModulo").val()+'&idPrueba='+$("#idRegistro").val();
				$.ajax({
					type: 'POST',
					url : url,		
	...
	$("#idPruebaBusca").on('change',function(event){
		var msg='';
		event.preventDefault();
		url   = baseUrl+'seguridad/regresaPerfiles';
		var idPrueba = $("#idPruebaBusca").val();
		if(parseInt(idPrueba) > 0 && String(token) !== '-1'){
			dataString = 'idPrueba='+idPrueba;	
			$.ajax({
				type: 'POST',
				url : url,		
				 headers: {
...
//		alert("opciones: " + opciones);
	...
	$(".editaComportamiento").on('click',function(event){
		...
		var msg='';
		event.preventDefault();
		url   = baseUrl+'prueba/regresaNivelesComportamiento';
	...
	$("#tipoInterprestacionPrueba").on('change',function(event){
		var msg='';
		event.preventDefault();
		var tipoInterpretacion = $("#tipoInterprestacionPrueba").val();
		if(tipoInterpretacion != '') {
		    $("#modalConfirmaInterpretacion").modal('show');
		} 
		if(tipoInterpretacion == 'C') {
		    $("#divCombinacion").show(); 
		    $("#divCombinacion").attr('style','width:80%; height:80%; overflow: scroll;');
		} else {
		    $("#divCombinacion").hide();
		}
	...
	$( document ).delegate(".confirmaTipoInterprestacionPrueba", "click", function() {
		var msg='';
		event.preventDefault();
		var tipoInterpretacion = $("#tipoInterprestacionPrueba").val();
		if(tipoInterpretacion != '') {
		    $("#divIngresaInterpretacion").show(); 
		    $("#tipoInterprestacionPrueba").attr("disabled","disabled");
		} else {
		    $("#divIngresaInterpretacion").hide();
		}
		 $("#modalConfirmaInterpretacion").modal('hide');
		return false;		
	});
	
	$( document ).delegate(".cancelaTipoInterprestacionPrueba", "click", function() {
		var msg='';
		event.preventDefault();
		$("#tipoInterprestacionPrueba").val("");
		return false;		
	});
	...
	$( document ).delegate("#btnAgregaOpcion", "click", function() {
		...
		var cadena = "<tr class='renglones' id='opcion-"+ orden +"' >" +
				...
				"<td width='30%'><input type='text' style='width:100%;' maxlength='300'></td>"+
...
//	    alert("confirmaAgregaResultado");
		var resultado = $("#resultado").val();
//		  alert("resultado: " + resultado);
		var descripcionResultado = $("#descripcionResultado").val();
//		alert("descripcionResultado: " + descripcionResultado);
		//Valida que existe pregunta
		if(resultado == "") {
...
//			"<a href='#' class='btn'> <button class='btn btn-primary editable  btn-danger' data-toggle='modal' data-target='#confirmaEliminaModal'> Eliminar </button></a></td>" +
	...
	$("#confirmaAgregaInterpretacion").click(function(){
//	    alert("confirmaInterpretacion");
		var interpretacion = $("#interpretacion").val();
//		  alert("interpretacion: " + interpretacion);
		var descripcionInterpretacion = $("#descripcionInterpretacion").val();
//		alert("descripcionInterpretacion: " + descripcionInterpretacion);
		//Valida que existe pregunta
		if(resultado == "") {
			$("#alert").text("Se requiere la interpretacion");
			$("#alert").attr("hidden", false);
			return false;
		}
		...
		if( $("#filaNo").val() == "null") {
			var nuevaFila="<tr>";
			nuevaFila+="<td style='visibility: hidden'>null</td>";
			nuevaFila+="<td>" + interpretacion + "</td>";
			nuevaFila+="<td>" + descripcionInterpretacion + "</td>";
			nuevaFila+="<td><a href='#' class='btn btn-default btn-xs'> <span data-toggle='tooltip' data-placement='top' title='Editar' class='glyphicon glyphicon-pencil'></span></a> " +
			"<a href='#' class='btn btn-default btn-xs modaleliminar'><span data-toggle='tooltip' data-placement='top' title='Eliminar' class='glyphicon glyphicon-trash'></span></a>" +
//			"<a href='#' class='btn'> <button class='btn btn-primary editable  btn-danger' data-toggle='modal' data-target='#confirmaEliminaModal'> Eliminar </button></a></td>" +
			"</td>";
			nuevaFila+="</tr>";
			$("#tablaInterpretacion").append(nuevaFila);
		} else {
			$("#fila-"+$("#filaNo").val()).children("td").each(function (index2) 
					{
				switch (index2) 
		...
		$("#agregaInterpretacionModal").modal('hide');
...
function abreAgregaInterpretacion(){
	$("#alert").attr("hidden", true);
	$("#interpretacion").val("");
	$("#descripcionInterpretacion").val("");
	$("#filaNo").val("null");
	return true;
}

function generaPrueba(){
//    alert("generaPrueba");
...
//	alert ("preguntas: " + preguntas);
    ...
    if(etapa == 3) {
//	//por registrar interpretacion
//	var cuantos = $('#tablaInterpretacion >tbody >tr').length ;
//	var id = "";
//	var contador = 0;
//	var interpretacion = "";
...
//
//	if (cuantos == 0) {
//		alert ("Se debe agregar por lo menos una interpretación");
//		return false;
//	}
//	
//	$("#tablaInterpretacion tbody tr").each(function (index) {
//	    $(this).children("td").each(function (index2) 
//		{
//		switch (index2) 
//			{
...
//
//
//	    interpretaciones = interpretaciones + id + "--"+ interpretacion + "--" + descripcion;
//
//		if(contador < cuantos - 1  )
...
//	});
//	
//	alert ("interpretaciones: " + interpretaciones);
//	
//	$("#interpretaciones").val(interpretaciones);
//	
    }
