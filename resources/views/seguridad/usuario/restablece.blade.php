	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
                <h4 class="modal-title">{{Lang::get('leyendas.restablece')}}</h4>
			</div>
			<div class="modal-body">
				<p>{{Lang::get('leyendas.confirmarestablece')}}</p>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-default btn-xs" data-dismiss="modal">{{Lang::get('leyendas.cerrar')}}</button>
				<button type="button" class="btn btn-success btn-xs restablecer" >{{Lang::get('leyendas.confirmar')}}</button>
			</div>
		</div>
	</div>
</div>