<div class="box box-solid">
	<div class="box-header">
    	...
    	<h3 class="box-title" style="color:blue;font-weight:bold;">Pruebas</h3>
	</div>
 	<div class="box-body">
 		...
 		<div class="rows">
        	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
        		<div class="form-group">
        			<label  class="col-lg-4 control-label" for="nombre">*{{Lang::get('leyendas.perfilPuesto.pruebas')}}</label>  
        				@if(count($pruebas) > 0)   		
        				<select name="idPruebaAgrega" id="idPruebaAgrega"  class="form-control talento" tabindex="3">
          					<option value="">{{Lang::get('leyendas.seleccione')}}</option>
        						@foreach($pruebas as $prueba)
        							<option value="{{$prueba->idprueba}}" >{{$prueba->nombre}}</option>
        						@endforeach  				
        				</select>
        				@endif
        		</div>
