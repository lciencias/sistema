...
class EnviaEmailReseteoPasswordJob  implements ShouldQueue
{
    ...
    public function handle(){
    	...
    	$this->log = \Logger::getLogger ( 'main' );
   		$this->log->info("Envio de mail de reset de contraseña: " . $this->user->email);
    	try {
	    	...
	    	Mail::send($template, $data, function($message) {
	    		try {
	    			...
	    			$this->log->info("Exito al enviar mail de reset de contraseña: " . $this->user->email);
	    		} catch ( \Exception $e ) {
    				$this->log->error("Error al enviar mail de reset de contraseña: " . $e->getMessage());
    				throw $e;
    			}
